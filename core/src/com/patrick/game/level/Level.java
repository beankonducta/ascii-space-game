...
public class Level {
    ...
    public void process(float delta, BitmapFont font, Batch batch) {
        ...
        for (Enemy enemy : this.waves.get(this.currentWave).getEnemies()) {
            ...
            MovementController.processEnemyMovement(enemy, this.player, CameraController.camera.viewportWidth);
        }

        this.player.update(delta);
        this.player.render(font, batch);
        player.move(CameraController.camera.viewportWidth);
        if(MovementController.processPlayerMovement(player))
            this.bullets.add(new Bullet(new Vector2(player.x(), player.y()), 20f, 0, '.'));
    }
