...
public class Wave {
    ...
    public Wave(boolean bossOnly, int difficulty) {
        if (bossOnly) {
            ...
            if(size > 5) size = 5;
            Boss boss = new Boss((CameraController.camera.viewportWidth / 2) - Math.randomBetween(-25, 25), (CameraController.camera.viewportHeight - 48) - Math.randomBetween(-25, 25), 50f, 25f, difficulty, Ascii.coolS(size - 1));
